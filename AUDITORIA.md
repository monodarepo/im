...
## 7. Onda 1 — execução (PD5, 03/08/2026)

Os quinze achados de esforço P foram tratados, na ordem de ID. Evidência
antes/depois em `docs/auditoria/onda-1/`. Verificação do AUD-01 nas quatro
larguras (1366/1440/1680/1920): zero sobreposição e altura única de card
(172px) na faixa — o AUD-09 exigiu três medidas além da prevista (zona de
rótulo com altura de duas linhas, atestado em linha única com truncagem, e
`whitespace-nowrap` no rodapé), todas dentro do escopo do achado.

Notas de resolução que registram decisão, não só correção:

- **AUD-19**: 10 ocorrências convertidas para `linear` em 8 arquivos; a curva
  preço×volume do Simulador de Elasticidade mantém `monotone` por ser função
  contínua teórica — exceção comentada no arquivo.
- **AUD-24**: no RootCause o badge inline virou `ProductBadge`. No
  DecisionDetail a ocorrência apontada é a barra de participação da parcela,
  que usa a cor do produto como identificação (mesmo papel do badge ao lado) —
  mantida deliberadamente.
- **AUD-30**: adotado o espaço fino U+2009 no `formatMoney` (e nas strings de
  moeda declaradas em mocks); 35 expectativas de teste atualizadas juntas.
- **AUD-13**: os quatro testes de estrutura de rota passaram a fixar
  rota + título; o mapeamento módulo↔rota vive em `/estilo`.

### Achados novos registrados durante a onda

| ID | Lente | Tela/arquivo | Achado | Por que importa | Sev. | Esf. |
|---|---|---|---|---|---|---|
| AUD-33 | 3 | raiz · `HYPERA_260729_ABS_01...docx` | O ESCOPO chegou ao repositório, mas é a edição conceitual de 26/07/29 ("Escopo da Ferramenta", seções 1–5+): **não contém** as seções 8/10/11/12.3 com números canônicos e schema de Decisão que os prompts das fases citavam. A seção 4 (Torre) confirma o que foi construído; os números canônicos seguem tendo como única fonte os prompts | A rastreabilidade número↔documento que o CFO pediria continua incompleta; falta o ABS_01 Consolidado de 26/07/30 | ALTO | — (depende de documento) |
| AUD-34 | 3 | `IntegratedTower` vs ESCOPO §4.2 | A lista dos dez indicadores da Torre diverge da lista do ESCOPO §4.2 (que pede: crescimento, share em volume, distribuição ponderada, cobertura de estoque, margem de contribuição e impacto das oportunidades em aberto — dos quais só parte aparece hoje, na linha secundária ou ausente) | Com o documento agora em mãos, a Torre pode ser cobrada contra ele na apresentação | MÉDIO | M |
